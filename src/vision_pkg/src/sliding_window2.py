...
def search_around_poly(binary_warped, left_fit, right_fit,
    ...
    if left_fit is not None:
        x_pred = np.polyval(left_fit, nonzeroy)
        ...
        if lc >= minlanepix:
            ...
            lc = len(ys)
        if lc >= minlanepix:
            lf_new = np.polyfit(ys, xs, cfg.POLY_ORDER)
    ...
    if right_fit is not None:
        x_pred = np.polyval(right_fit, nonzeroy)
        mask = (np.abs(nonzerox - x_pred) < margin_arr)
        rc = int(np.sum(mask))
        if rc >= minlanepix:
            idx = np.where(mask)[0]
            ...
            rc = len(ys)
        if rc >= minlanepix:
            rf_new = np.polyfit(ys, xs, cfg.POLY_ORDER)
